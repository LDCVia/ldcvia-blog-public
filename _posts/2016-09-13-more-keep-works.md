...
We’ve been asked numerous times to explain a little more about what [KEEP.WORKS](https://keep.works) is, what it does, what it doesn’t do, how it works, and who it’s for.

Before tackling those, let’s talk about why we’ve created it.

#### Why KEEP.WORKS?
There are two main reasons. Firstly: IBM Lotus Notes - there’s a lot of it about, but it’s also on the endangered species list. By which we mean there are many individuals and teams whose Notes applications - that they rely on to run their job/work - are being removed or threatened by an organisation shutting down their IBM Lotus Notes and Domino infrastructure. So [KEEP.WORKS](https://keep.works) is designed to help them have a low-cost migration path. At the very least, it lets them retain their data, safe from the destructive urges of their management and IT people. But more, it lets them continue to work with that data (more on this later). And it’s sufficiently low-cost that it can all pass through on expenses or a company credit card.

The second reason for [KEEP.WORKS](https://keep.works) is that we’ve found that some people have understandably struggled with the migration tooling we provide to move data into [LDC Via](https://ldcvia.com). That’s been down to a mixture of Domino complexities and [LDC Via](https://ldcvia.com) complexities, but it has been a stumbling block. So [KEEP.WORKS](https://keep.works) is a way of removing that barrier by having us do the heavy lifting. It’s limited to small-ish NSFs - up to 2Gb and 50,000 documents - but that’s plenty for many uses, and sufficient for a serious pilot in the case of larger applications.
...
* People or organisations wanting to ‘try’ the [LDC Via](https://ldcvia.com) platform with as little learning curve as possible
* Those looking to bring simple Notes client apps to the web

#### What does it do?
As the website says: you upload your NSF, we convert it onto the [LDC Via](https://ldcvia.com) platform automagically, and then you access and work with your data via a browser or mobile device.
...
It can take as little as 10 minutes to move data from Notes/Domino into [KEEP.WORKS](https://keep.works), and have it operational on the cloud and fully mobile-enabled. Additional users can be created using the [KEEP.WORKS](https://keep.works) application.
...
Our approach, as with [LDC Via](https://ldcvia.com), is to retain the data in as perfect a form as reasonably possible, while removing the reliance on legacy server platforms and obscure and outdated software development tools. We provide a simple point-and-click interface to allow you to define the form layout. Over time we will be providing more options, in response to user feedback, but we wouldn’t want to set up the expectation that you can develop a complex multi-faceted application entirely within [KEEP.WORKS](https://keep.works): that’s not what this product is for.

#### How does it work?
[KEEP.WORKS](https://keep.works) is an application built on top of the [LDC Via](https://ldcvia.com) platform. It’s not a core part of [LDC Via](https://ldcvia.com), which is why it has its own name and website, and is hosted separately from [LDC Via](https://ldcvia.com). Everything we’ve done on the [KEEP.WORKS](https://keep.works) site uses the normal published [LDC Via REST-based APIs](https://api.ldcvia.com) to do its work. You could subscribe to [LDC Via](https://ldcvia.com) and go out and build a [KEEP.WORKS](https://keep.works) type tool yourself, if you so chose (with the one minor caveat of the database conversion process that runs behind the scenes - that’s not exposed as API calls).  


#### What next?
Well, if you’re one of those who made it onto our beta programme - congratulations, and you will receive an email this week detailing how to create your account. If you’re not … you’ll have to wait until we go fully live in a few weeks time. You can register your interest in [KEEP.WORKS](https://keep.works) at [https://keep.works/#register](https://keep.works/#register).
